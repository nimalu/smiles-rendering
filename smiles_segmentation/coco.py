...
def extract_coco_label(svg_path: Path, image_path: str, image_id: str, class_to_id: dict[str, int]) -> dict:
    ...
    for instance_id in instance_ids:
        ...
        for el in elements:
            if el.tag.endswith("polygon"):
                ...
                if not points:
                    raise ValueError(f"Polygon element with instance-id '{instance_id}' has no points attribute")
                points = points.strip().split(" ")
                points = [tuple(map(float, p.split(","))) for p in points]
                points = [(round(x), round(y)) for x, y in points]
                paths.append(points)
            else:
                raise ValueError(f"Unsupported SVG element '{el.tag}' for instance-id '{instance_id}'")
